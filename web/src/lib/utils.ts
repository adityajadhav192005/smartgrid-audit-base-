...
export function severityColor(severity: string) {
  switch (severity?.toLowerCase()) {
    ...
    case 'medium':   return 'text-amber-600 bg-amber-400/10 border-amber-400/20'
    case 'low':      return 'text-emerald-600 bg-emerald-50 border-emerald-400/20'
...
export function statusColor(status: string) {
  switch (status?.toLowerCase()) {
    ...
    case 'healthy':
    case 'normal':
    case 'mitigated':  return 'text-emerald-600'
    case 'auditing':
