...
export function TopBar() {
  ...
  return (
    <div className="h-11 flex items-center px-4 gap-4 bg-white border-b border-slate-200 flex-shrink-0 relative">
      {/* Search */}
      <div className="flex items-center gap-2 flex-1 max-w-sm">
        ...
        <input
          ...
          className="bg-transparent text-sm text-slate-800 placeholder:text-slate-500 outline-none flex-1"
        />
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded border border-slate-300 text-slate-700 text-xs hover:border-slate-400 hover:text-slate-900 transition-colors"
        >
      ...
      {showNotifications && (
        <div className="absolute right-4 top-12 z-50 w-80 max-h-80 overflow-auto glass-card border border-slate-200 p-3 shadow-lg">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-semibold text-slate-700">Notifications</span>
            <button onClick={clearNotifications} className="text-[10px] text-slate-500 hover:text-slate-900">Clear</button>
          </div>
          <div className="space-y-2">
            ...
            {notifications.map(n => (
              <div key={n.id} className="rounded border border-slate-200 bg-slate-50 p-2">
                <div className="text-xs text-slate-800 font-semibold">{n.title}</div>
                <div className="text-[11px] text-slate-600 mt-0.5">{n.message}</div>
                <div className="text-[10px] text-slate-500 mt-1">{new Date(n.ts).toLocaleTimeString()}</div>
              </div>
            ))}
          </div>
        </div>
