...
function toAuditRecord(event: { id: string; ts: string; type: string; msg: string; severity: string }, index: number): AuditRecord {
  ...
  return {
    id: `EXP-AUD-${String(index + 1).padStart(3, '0')}`,
...
export default function ExperimentAuditsPage() {
  const { events, statusCounts } = useExperimentTelemetry(8000)
  const records = events.filter(e => e.type.includes('AUDIT') || e.type.includes('ANOM') || e.type.includes('ATTACK')).map((e, i) => toAuditRecord(e, i))
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="section-header">Experiment Audit Trail</h1>
        <p className="text-sm text-slate-400 mt-1">Audit decisions, evidence flow, and anomaly-triggered response records from the experiment</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <KPIStatCard label="Resolved" value={events.filter(e => e.msg.toUpperCase().includes('MAINTAIN')).length} color="green" icon={<CheckCircle size={14} />} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        <div className="lg:col-span-2 glass-card p-0 overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-700/50 text-sm font-semibold text-slate-200">Experiment Audits</div>
        ...
        <div className="lg:col-span-3 glass-card p-5 space-y-4">
          {!selectedRecord ? (
            <div className="text-sm text-slate-400">No experiment audit records yet. Launch a run to generate data.</div>
          ) : (
            <>
