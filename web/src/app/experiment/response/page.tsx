...
function buildResponseSteps(events: { ts: string; type: string; msg: string }[]) {
  ...
  if (!latest) {
    return [
      { step: 1, action: 'Awaiting Experiment Data', icon: '...', detail: 'Launch a run to see response workflow', time: 'Pending', done: false },
    ]
  }
  return [
    { step: 1, action: 'Deviation Detected', icon: 'DET', detail: latest.msg, time: latest.ts, done: true },
    { step: 2, action: 'Audit Decision Raised', icon: 'AUD', detail: 'Audit event recorded for the affected agent', time: latest.ts, done: true },
    { step: 3, action: 'Severity Classified', icon: 'CLS', detail: `Decision path derived from ${latest.type}`, time: latest.ts, done: true },
    { step: 4, action: 'Mitigation Applied', icon: 'ACT', detail: 'Response policy applied using current severity and risk state', time: latest.ts, done: true },
  ]
}

export default function ExperimentResponsePage() {
  const { events, health, statusCounts, summary } = useExperimentTelemetry(8000)
  const responseSteps = buildResponseSteps(events)
  const riskMitigation = Number(summary?.riskMitigation ?? Math.max(0, 1 - Number(health.risk ?? 0)))
  const avgResponseSeconds = events.length > 0 ? 3 : 0
  const threatsResolved = events.filter(e => e.msg.toUpperCase().includes('MAINTAIN')).length
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="section-header">Experiment Response Workflow</h1>
        <p className="text-sm text-slate-400 mt-1">Response sequence, mitigation actions, and policy outcomes from the experiment run</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="glass-card p-4">
          <h3 className="text-sm font-semibold text-slate-200 mb-4">Response Event Log</h3>
          <div className="space-y-2">
            {events.length === 0 ? (
              <div className="text-sm text-slate-500">No experiment events yet.</div>
            ) : events.map(event => (
