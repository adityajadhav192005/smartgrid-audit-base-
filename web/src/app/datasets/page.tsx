  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Real-world and synthetic data sources for training and evaluation</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {datasets.map(ds => (
          <div key={ds.id} className={`glass-card p-4 space-y-3 border transition-colors ${ds.active ? 'border-cyber-green/30' : 'border-slate-200'}`}>
            ...
            <div className="flex flex-wrap gap-4 text-xs text-slate-500">
              <span>Rows: <span className="text-slate-700">{ds.rows.toLocaleString()}</span></span>
              <span>Features: <span className="text-slate-700">{ds.features}</span></span>
              <span>Split: <span className="text-slate-700">{ds.split}</span></span>
            </div>
          </div>
        ))}
      </div>
