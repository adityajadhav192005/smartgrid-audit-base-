    ...
    <article className="space-y-10 max-w-5xl mx-auto pb-16">
      ...
      <header className="border-b border-slate-200 pb-6">
        ...
        <div className="mt-3 text-xs text-slate-500 space-x-2">
          <span>Run: <span className="font-mono text-slate-700">{runId}</span></span>
          <span>·</span>
          <span>Mode: <span className="font-mono text-slate-700">{ablation}</span></span>
          <span>·</span>
          <span>Profile: <span className="font-mono text-slate-700">{profile}</span></span>
          <span>·</span>
          <span>Agents: <span className="font-mono text-slate-700">{totalAgents}</span></span>
        </div>
      </header>
              ...
              {HEADLINE.map(row => {
                ...
                return (
                  <tr key={row.metric}>
                    ...
                    <td className="font-mono text-slate-900">{oursStr}</td>
                    <td className="font-mono text-slate-500">{paperStr}</td>
                    <td className="font-mono text-slate-700">{delta || '—'}</td>
                    <td className={`pr-4 ${verdict === 'improvement' ? 'text-emerald-600' : verdict === 'below paper' ? 'text-rose-400' : 'text-slate-500'}`}>
                      {verdict || 'new metric'}
                    </td>
                  </tr>
                )
                ...
                {ATTACK_ORDER.filter(name => perAttack[name]).map(name => {
                  ...
                  return (
                    <tr key={name}>
                      ...
                      <td className="font-mono text-slate-500">{m.support}</td>
                      <td className="pr-4 font-mono text-slate-500">{m.predictedSupport}</td>
                    </tr>
                  )
                })}
              </tbody>
              ...
              <tbody>
                {Object.entries(stats).map(([key, v]) => (
                  <tr key={key}>
                    ...
                    <td className="text-slate-500">{v.test.replace(/_/g, ' ')}</td>
                    <td className="font-mono">{v.pValue == null ? 'n/a' : v.pValue < 1e-6 ? v.pValue.toExponential(2) : v.pValue.toFixed(4)}</td>
                    <td className={`pr-4 ${v.significant ? 'text-emerald-600' : 'text-slate-500'}`}>{v.significant ? 'yes' : 'no'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <table className="report-table">
              ...
              <tbody>
                <tr>
                  ...
                  <td className="pr-4 text-slate-500">ε final = {conv.rlEpsilonFinal.toFixed(3)}</td>
                </tr>
                <tr>
                  ...
                  <td className="pr-4 text-slate-500">refines magnitudes from RL direction</td>
                </tr>
              </tbody>
            </table>
          </div>
            ...
            <tbody>
              {CONTRIBUTIONS.map(c => (
                <tr key={c.id}>
                  ...
                  <td className="font-mono text-[11px] text-slate-500 max-w-xs truncate" title={c.where}>{c.where}</td>
                  <td className="text-slate-500 text-xs leading-snug max-w-md">{c.why}</td>
      ...
      <section className="space-y-3">
        ...
        <ul className="text-sm text-slate-500 leading-relaxed space-y-2 list-disc pl-5">
          ...
          <li>
            ...
            (<code className="text-slate-700 bg-slate-50 px-1 rounded">lstm_pretrained.pt</code>,
            {' '}<code className="text-slate-700 bg-slate-50 px-1 rounded">lstm_network.pt</code>) are real and
            shipped in <code className="text-slate-700 bg-slate-50 px-1 rounded">smartgrid_mas/data/anomaly_inputs/</code>.
          </li>
        </ul>
      </section>
    </article>
