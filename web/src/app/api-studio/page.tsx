  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">Explore and test FastAPI backend endpoints — proxied at <code className="text-cyber-blue text-xs">/api/backend/*</code></p>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4" style={{ minHeight: '60vh' }}>
        {/* Endpoint list */}
        <div className="glass-card p-0 overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-700/60 text-xs font-semibold text-slate-400 uppercase tracking-wider">Endpoints</div>
          <div className="overflow-y-auto" style={{ maxHeight: '68vh' }}>
            {groups.map(g => (
              <div key={g}>
                <div className="px-4 py-1.5 text-xs font-bold text-slate-500 uppercase tracking-wider bg-grid-900/40">{g}</div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="glass-card p-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <code className="text-sm text-slate-200 font-mono flex-1">/api/backend{selected.path}</code>
            </div>
            <p className="text-xs text-slate-400">{selected.desc}</p>
            <div className="flex items-center gap-2 pt-1">
              <div className="flex-1 bg-grid-900 rounded px-3 py-1.5 text-xs font-mono text-slate-400 border border-slate-700/40">
          ...
          <div className="glass-card p-4 space-y-2">
            ...
            <div className="bg-grid-900 rounded border border-slate-700/40 p-4 min-h-[200px] font-mono text-xs">
              {loading ? (
