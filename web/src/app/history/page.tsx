  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">Recent experiment runs, backend-reported metrics, and comparison trends</p>
      </div>
      ...
      <div className="glass-card p-4">
        <h3 className="text-sm font-semibold text-slate-200 mb-3">Run Comparison</h3>
        <div className="h-56">
      ...
      <div className="glass-card overflow-auto">
        <table className="w-full text-xs">
          ...
          <tbody>
            {runs.map(run => (
              <tr key={`${run.runId ?? 'live'}-${run.status}-${run.totalAgents}`} className="data-table-row border-b border-slate-800/40">
                ...
                <td className="px-4 py-2.5 text-slate-300 font-mono">{run.totalAgents}</td>
                <td className="px-4 py-2.5 text-slate-400">{run.attacksDetected}</td>
                <td className="px-4 py-2.5 text-slate-400">{run.attacksResolved}</td>
                <td className="px-4 py-2.5 text-slate-400">{run.auditsTriggered}</td>
                <td className="px-4 py-2.5 text-slate-300">{formatPct(run.costEfficiency)}</td>
                <td className="px-4 py-2.5 text-slate-300">{formatPct(run.riskMitigation)}</td>
                <td className="px-4 py-2.5 text-slate-300">{formatPct(run.detectionAccuracy)}</td>
                <td className="px-4 py-2.5 text-slate-400">{run.runtimeSeconds > 0 ? `${run.runtimeSeconds.toFixed(1)}s` : '-'}</td>
                <td className="px-4 py-2.5 text-slate-400">{run.convergenceIterations > 0 ? run.convergenceIterations : '-'}</td>
              </tr>
            ))}
          </tbody>
        </table>
