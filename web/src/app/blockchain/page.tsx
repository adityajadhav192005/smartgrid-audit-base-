...
const SEV_STYLE: Record<string, string> = {
  ...
  LOW:      'text-emerald-400 bg-emerald-900/15 border-emerald-500/25',
}
...
export default function BlockchainPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Demo mode notice */}
      {!isLive && !loading && (
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-amber-950/20 border border-amber-500/20 text-amber-300 text-xs">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Chain Integrity', value: status?.chain_ok ? 'VALID' : 'BROKEN',      color: status?.chain_ok ? 'text-emerald-400' : 'text-red-400', Icon: Shield },
          { label: 'Critical Events', value: critCount,                                  color: critCount > 0 ? 'text-red-400' : 'text-slate-400', Icon: AlertTriangle },
          { label: 'Backend',         value: status?.backend ?? '—',                     color: 'text-slate-300',   Icon: Cpu        },
      ...
      <div className="flex gap-4">
        ...
        <div className="glass-card p-4 flex-1 space-y-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-slate-200">Event Chain</span>
            {status?.latest_hash && (
                ...
                <div key={ev.event_id} className="relative group">
                  ...
                  <div className={cn(
                    ...
                    detail?.event_id === ev.event_id
                      ...
                      : 'border-slate-700/25 bg-grid-900/30 hover:border-slate-600/40'
                  )} onClick={() => setDetail(detail?.event_id === ev.event_id ? null : ev)}>

                    {/* Block icon */}
                    <div className="w-9 h-9 rounded-lg bg-slate-800/60 border border-slate-700/40 flex flex-col items-center justify-center flex-shrink-0">
                    ...
                    <div className="flex-1 min-w-0 space-y-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="text-xs font-bold text-slate-100">{ev.agent_id}</span>
                      ...
                      <div className="flex gap-3 text-[9px] font-mono text-slate-500 flex-wrap">
                        <span>prev: <span className="text-slate-400">{ev.prev_hash.slice(0, 10)}…</span></span>
                        <span>hash: <span className="text-cyan-500">{ev.chain_hash.slice(0, 10)}…</span></span>
                        {ev.payload?.deviation_score !== undefined && (
                          <span>score: <span className={Number(ev.payload.deviation_score) > 1 ? 'text-red-400' : 'text-emerald-400'}>
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 text-[9px] px-2 py-1 rounded border border-slate-700/40 text-slate-500 hover:border-cyan-500/40 hover:text-cyan-400 transition-all opacity-0 group-hover:opacity-100 disabled:opacity-40 flex-shrink-0">
                      {verifying === ev.event_id
                        ? <RefreshCw size={9} className="animate-spin" />
                        : verified[ev.event_id] !== undefined
                          ? (verified[ev.event_id] ? <CheckCircle2 size={9} className="text-emerald-400" /> : <XCircle size={9} className="text-red-400" />)
                          : <Shield size={9} />}
                  ...
                  {/* Expanded detail */}
                  {detail?.event_id === ev.event_id && (
                    <div className="mx-3 mt-1 p-3 rounded-b-lg bg-cyan-950/10 border border-t-0 border-cyan-500/20 text-[10px] font-mono text-slate-400 space-y-1">
                      <div className="grid grid-cols-2 gap-x-6 gap-y-1">
                        <span>event_id: <span className="text-slate-200">{ev.event_id}</span></span>
                        <span>event_type: <span className="text-slate-200">{ev.event_type}</span></span>
                        <span>agent_id: <span className="text-slate-200">{ev.agent_id}</span></span>
                        <span>severity: <span className={SEV_STYLE[ev.severity]?.split(' ')[0]}>{ev.severity}</span></span>
                        <span>timestamp: <span className="text-slate-200">{ev.timestamp}</span></span>
                      </div>
                      <div className="border-t border-slate-700/30 pt-1 space-y-0.5">
                        <div>prev_hash: <span className="text-slate-300">{ev.prev_hash}</span></div>
        ...
        <div className="w-52 flex-shrink-0 flex flex-col gap-3">
          ...
          <div className="glass-card p-3 space-y-2">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-semibold text-slate-300">Severity Breakdown</span>
            </div>
          ...
          <div className="glass-card p-3 space-y-2">
            ...
            {[
              ...
              <div key={r.label} className="flex justify-between text-[10px]">
                ...
                <span className={cn('font-mono font-semibold', r.ok ? 'text-emerald-400' : 'text-amber-400')}>{r.value}</span>
              </div>
            ))}
          </div>
