...
% Tight spacing for 8-page limit
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.78\columnwidth]{figs/fig_tier_arch.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.92\columnwidth]{figs/fig_flow.png}}
\caption{End-to-end system flow: Rapid SCADA telemetry acquisition, multi-layer detection pipeline, hybrid audit scheduler, explainability module, and Next.js dashboard.}
\label{fig:flow}
\end{figure}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.92\columnwidth]{figs/fig_detection_arch.png}}
\caption{Multi-layer detection architecture. Layer~A computes per-agent deviation scores. Layer~B applies the dual-branch LSTM. Layer~C fuses both through weighted combination. The three-detector module runs alongside and feeds into the OR-with-precedence combiner.}
...
\label{eq:ensemble}
\end{equation}
We picked $\alpha$ and $\beta$ via grid search on a held-out validation split. The LSTM branch gets slightly more weight ($\beta > \alpha$) because it offers better specificity once temporal context accumulates, while the deviation score reacts faster in the first few time steps.
...
\label{eq:explain}
\end{equation}
The top-$k$ features, ranked by $|\phi_j^{(i)}|$, are displayed in the decision explainability view so an operator can see exactly which measurement dimensions drove the alert.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.82\columnwidth]{figs/fig_exp_ops.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.82\columnwidth]{figs/fig_explainability.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.78\columnwidth]{figs/fig_method_comparison.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.78\columnwidth]{figs/fig_radar.png}}
...
Isolation Forest picks up 53.92\% of attacks (best recall among the standalone methods) but at a painful 27.95\% FPR. One-Class SVM is the weakest overall: 46.35\% accuracy, 59.99\% FPR. It does achieve 75.93\% recall, but only because it labels most readings as anomalous.

Our multi-layer pipeline tops all five metrics: 94.23\% accuracy, 1.81\% FPR, 75.76\% recall, 89.95\% precision, 82.25\% F1. The F1 gap over the next-best single method is 44.33 pp, which confirms that layering deviation scoring, LSTM temporal context, and attack-specific detectors together gives coverage that no individual technique can match.

\subsection{Scalability Evaluation}

Table~\ref{tab:scale} shows how the system behaves at three scales. Accuracy holds at 99.28\% for N=200 but drops to 82.85\% at N=500 as pipeline serialisation becomes the bottleneck.
...
At N=200, nearly everything still works: coverage is 99.50\%, accuracy barely dips, and the FPR stays under 1\%. At N=500, the picture changes. The 17.63\% FPR tells us that detection thresholds calibrated for 100 agents no longer fit a population five times larger. The current sequential architecture works well up to about 200 agents; going beyond that will require batched LSTM inference, per-scale threshold tuning, or distributed processing.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.82\columnwidth]{figs/fig_scada_ops.png}}
\caption{Rapid SCADA live workspace: all 100 agents active and monitored in real time through the PowerShell bridge.}
\label{fig:scada_ops}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.82\columnwidth]{figs/fig_live_trace.png}}
