...
**Current Status (as of March 2026 - v8, validated):**
- N=100: Cost Efficiency 83.55%, Risk Mitigation 67.62%, Accuracy 99.56%, Precision/Recall 0.2515/1.0
- N=200: Cost Efficiency 84.75%, Risk Mitigation 71.33%, Accuracy 99.55%, Precision/Recall 0.2362/1.0
- N=500: Cost Efficiency 92.65%, Risk Mitigation 72.08%, Accuracy 99.54%, Precision/Recall 0.2278/1.0
- Status summary: paper headline metrics are exceeded; precision target (0.35+) still pending.
...
- **Reward Function (CRITICAL - UPDATED March 2026 v2.0 - HOTFIX)**:
  - **ISSUE FIXED**: Previous version (v1.0) had inverted penalty weights causing -5.98% risk mitigation
  - **OLD (BROKEN)**: λ_audit=0.5 (cost), λ_attack=2.0 (security) → agent learned to minimize audits
  - **NEW (FIXED)**: λ_audit=0.2 (cost penalty DECREASED), λ_attack=5.0 (security penalty INCREASED)
  - Implementation (smartgrid_mas/environment/reward_function.py):
    - α₁ (False Positive penalty) = lambda_audit = 0.2 (reduced from 0.5)
    - α₂ (False Negative penalty) = lambda_attack = 5.0 (increased from 2.0)
    ...
    - Total reward: -cost_objective - detection_penalty + detect_bonus (security-first formulation)
  - **Expected Impact**: Risk mitigation should shift from -5.98% → +10-15% (paper target)
...
### Fix #6: Reward Function Weight Inversion (IN PROGRESS - March 1, 2026)
**Problem**: RL agent learned to minimize audits aggressively because cost penalty outweighed security penalty
- Result: Risk mitigation NEGATIVE (-5.98%) instead of positive (+10-15% target)
- Root cause: λ_audit = 0.5 > λ_attack = 2.0 made cost minimization dominant objective
- Symptom: 90% cost efficiency (good) but -5.98% risk mitigation (bad) = inverted priorities

**Solution Implemented**:
1. Increased missed_attack penalty: λ_attack = 2.0 → 5.0 (security-critical)
2. Decreased audit_cost penalty: λ_audit = 0.5 → 0.2 (cost secondary)
3. Updated alpha_2 to use lambda_attack instead of hardcoded 2.0 (consistency)
4. File modified: `smartgrid_mas/environment/reward_function.py` (lines 15-18, 70-72)

**Expected Impact**: Risk mitigation should improve from -5.98% → +10-15% (matches paper)

**Next Steps**:
- Re-train RL agent with new weights
- Monitor risk mitigation metric for positive shift
- Validate precision improvement (currently 0.21-0.24, target 0.35+)
- Run full N-sweep validation (N=100,200,500)

### Fix #7: Hidden Hard-Constraint Behavior (RESOLVED - March 2026)
**Problem**: Dynamic execution had an implicit hard budget path causing suspicious fixed-style outputs in some runs.

**Solution**:
1. Made execution hard budget opt-in via `SMARTGRID_EXEC_HARD_BUDGET=1`
2. Default execution uses soft budget behavior
3. Added dynamic per-step minimum override for dynamic scheduler (`SMARTGRID_DYNAMIC_F_MIN_PER_STEP`, default 0)

**Impact**: Removed fixed-cost artifact behavior and restored genuine dynamic spend variation across N.

### Fix #8: Per-Attack Metrics Accounting (RESOLVED - March 2026)
**Problem**: Per-attack metrics were computed on positive-only slices, making TNR/FPR/accuracy misleading and causing class-level confusion.

**Solution**:
1. Reworked `per_attack_confusion` to use full one-vs-rest confusion over the complete sample set
2. Added label normalization for attack-type matching
3. Added `support` and `predicted_support` fields per class

**Impact**: Scenario-level metrics are now structurally valid for paper-style reporting.
...
- Risk Mitigation: 10-15% minimum (POSITIVE = RL better than baseline)
- Cost Efficiency: 60-75% minimum (higher is acceptable only if security metrics remain strong)
- Detection Accuracy: >90% (target >95%)
- Precision target: 0.35+ (current key gap)
- All must be reported for N=100,200,500

### ⚠️ MANDATORY: Step-Gate Protocol for Remaining Work
Before moving to any new optimization step (including Step 3+), enforce these gates:

1. **Gate A (headline paper parity)**: cost efficiency, coverage, and accuracy must remain at/above paper baseline.
2. **Gate B (per-attack validity)**: per-attack tables must include `support` values; avoid claiming performance for classes with near-zero support.
3. **Gate C (robustness)**: run multi-seed (≥3 seeds) and report mean ± std for primary metrics.
4. **Gate D (ablation integrity)**: HYBRID vs RL_ONLY vs GRADIENT_ONLY must be reported under identical scenario settings.
5. **Gate E (reproducibility)**: preserve exact env vars/commands used for final numbers.
